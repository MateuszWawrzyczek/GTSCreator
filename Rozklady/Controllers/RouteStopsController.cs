...
public class RouteStopsController : ControllerBase
{
    ...
    public async Task<IEnumerable<RouteStopsDto>> GetRoutesStops([FromQuery] string feedId, [FromQuery] string routeId)
    {
        ...
        var sql = @"WITH trip_variants AS (
                ...
                GROUP BY t.feed_id, t.route_id, t.direction_id, st.trip_id
            ),
                            unique_variants AS (
                SELECT 
                    feed_id AS FeedId,
                ...
                FROM trip_variants
                GROUP BY feed_id, route_id, direction_id, stop_pattern
            )
            SELECT
                uv.VariantId,
                uv.FeedId,
            ...
            GROUP BY uv.VariantId, uv.FeedId, uv.RouteID, uv.DirectionId
